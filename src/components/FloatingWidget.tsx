...
export const FloatingWidget: React.FC<FloatingWidgetProps> = ({
  ...
  return (
    <motion.button
      ...
      className={cn(
        "fixed bottom-24 right-4 sm:right-[calc(50%-210px)] w-11 h-11 rounded-full flex items-center justify-center bg-orange-500 hover:bg-orange-600 text-white shadow-lg shadow-orange-500/25 z-40 border-none cursor-pointer transition-all active:scale-95 select-none"
      )}
      title={getLabel()}
    >
      {renderIcon()}
